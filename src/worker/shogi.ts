...
export function exportGameAsKif(game: StoredGame, users: Map<string, UserSummary>): string {
  const snapshot = snapshotFromStoredGame(game, users);
  const lines = [
    "# KIF version=2.0 encoding=UTF-8",
    `# Generated by shogiwoshiyou`,
    `開始日時：${formatKifDate(snapshot.createdAt)}`,
    `終了日時：${formatKifDate(snapshot.updatedAt)}`,
    "手合割：平手",
    `先手：${snapshot.players.black.displayName}`,
    `後手：${snapshot.players.white?.displayName ?? "相手待ち"}`,
    "手数----指手---------消費時間--",
  ];
  for (const move of snapshot.moves) {
    lines.push(`${move.ply.toString().padStart(4, " ")} ${move.notation} (${move.usi})`);
  }
  lines.push(`${(snapshot.moves.length + 1).toString().padStart(4, " ")} ${kifEndLabel(snapshot)}`);
  return `${lines.join("\n")}\n`;
}
...
function formatKifDate(value: string): string {
...
function kifEndLabel(snapshot: GameSnapshot): string {
  if (snapshot.endReason === "resign") {
    return "投了";
  }
  if (snapshot.endReason === "checkmate") {
    return "詰み";
  }
  if (snapshot.endReason === "draw") {
    return "持将棋";
  }
  if (snapshot.endReason === "timeout") {
    return "時間切れ";
  }
  if (snapshot.endReason === "foul") {
    return "反則";
  }
  return snapshot.status === "ended" ? "終局" : "中断";
}
